import { upload } from './googlephotos';
...
function requiredFeaturesSupported() {
  ...
  if (!window.indexedDB.deleteDatabase) { // Not all implementations of IndexedDB support this method, thus we test for it here.
    ...
    //document.getElementById('downloadButton').addEventListener('click', downloadRecord, false);
  return true;
...
function handleFileSelection(evt) {
  ...
  displayMessage("<p>handleFileSelection</p>");
  ...
  if (!db) {
    ...
    displayMessage("<p>db (i.e., dbGlobals.db) is null in handleFileSelection()</p>");
    return;
  } // if
  ...
  try {
    ...
    displayMessage("<p>db.transaction exception in handleFileSelection() - " + ex.message+"</p>");
    return;
  } // catch

  transaction.onerror = function(evt) {
    displayMessage("<p>transaction.onerror fired in handleFileSelection() - error code: " + (evt.target.error ? evt.target.error : evt.target.errorCode)+"</p>");
  ...
  transaction.onabort = function() {
    displayMessage("<p>transaction.onabort fired in handleFileSelection()</p>");
  ...
  transaction.oncomplete = function() {
    ...
    displayMessage("<p>transaction.oncomplete fired in handleFileSelection()</p>");
  ...
  try {
    ...
    for (var i = 0, file; file = files[i]; i++) {
        ...
        reader.onload = function(e) {
            ...
            var putRequest = objectStore.put(ob);
      ...
      //var putRequest = objectStore.put(file); // The put() method will update an existing record, whereas the add() method won't.
            ...
            putRequest.onerror = function(evt) {
                displayMessage("<p>putRequest.onerror fired in handleFileSelection() - error code: " + (evt.target.error ? evt.target.error : evt.target.errorCode)+"</p>");
    ...
    displayMessage("<p>Transaction and/or put() exception in handleFileSelection() - " + ex.message+"</p>");
    ...
    try {
      ...
      cursorRequest.onsuccess = function(evt) {
        ...
        if (cursor) {
          ...
          fileListHTML += "<li>ID " + cursor.value.ID + " - " + cursor.value.name;
          fileListHTML += "<p style='margin: 0 0 0 0.75em;'> created: " + cursor.value.created + "</p>";
          fileListHTML += "<p style='margin: 0 0 0 0.75em;'> referentie: " + cursor.value.referentie + "</p>";
...
export { requiredFeaturesSupported, uploadRecord };